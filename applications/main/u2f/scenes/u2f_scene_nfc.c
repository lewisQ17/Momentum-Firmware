...
// NFC transport: the U2F core is transport-agnostic, so this scene mirrors the
// USB (main) scene one-to-one and only swaps u2f_hid_* for u2f_nfc_*.

#define U2F_REQUEST_TIMEOUT 500
#define U2F_SUCCESS_TIMEOUT 3000

static void u2f_scene_nfc_ok_callback(InputType type, void* context) {
    UNUSED(type);
    furi_assert(context);
    U2fApp* app = context;
...
static void u2f_scene_nfc_event_callback(U2fNotifyEvent evt, void* context) {
    furi_assert(context);
    U2fApp* app = context;
...
static void u2f_scene_nfc_timer_callback(void* context) {
    furi_assert(context);
    U2fApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, U2fCustomEventTimeout);
}

bool u2f_scene_nfc_on_event(void* context, SceneManagerEvent event) {
    furi_assert(context);
    U2fApp* app = context;
    bool consumed = false;
...
void u2f_scene_nfc_on_enter(void* context) {
    U2fApp* app = context;

    app->timer = furi_timer_alloc(u2f_scene_nfc_timer_callback, FuriTimerTypeOnce, app);

    app->u2f_instance = u2f_alloc();
    app->u2f_ready = u2f_init(app->u2f_instance);
    if(app->u2f_ready == true) {
        u2f_set_event_callback(app->u2f_instance, u2f_scene_nfc_event_callback, app);
        app->u2f_nfc = u2f_nfc_start(app->u2f_instance);
        u2f_view_set_ok_callback(app->u2f_view, u2f_scene_nfc_ok_callback, app);
    } else {
...
void u2f_scene_nfc_on_exit(void* context) {
    U2fApp* app = context;
    notification_message_block(app->notifications, &sequence_reset_rgb);
    furi_timer_stop(app->timer);
