...
void momentum_app_scene_misc_on_enter(void* context) {
    ...
    item = variable_item_list_add(var_item_list, "Credits", 0, NULL, app);
    variable_item_set_current_value_text(item, ">");
...
bool momentum_app_scene_misc_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        case VarItemListIndexMemInfo:
            ...
            break;
        case VarItemListIndexCredits:
            scene_manager_set_scene_state(app->scene_manager, MomentumAppSceneMiscCredits, 0);
            scene_manager_next_scene(app->scene_manager, MomentumAppSceneMiscCredits);
            break;
